...
static int line(const char *text, const char *key, char *out, int max) {
    ...
    while(p && *p) {
        ...
        if(match && (p[k]==':' || p[k]=='=')) { int off=k+1; while(off<i && (p[off]==' '||p[off]=='\t')) ++off; copy_value(out,max,p+off); return 1; }
        p += i; if(*p=='\n') ++p;
    }
    return 0;
}
int nexus_package_manifest_parse(const char *text, nexus_package_manifest_t *out) {
    ...
    out->name[0]=out->id[0]=out->version[0]=out->author[0]=out->type[0]=out->entry[0]=out->icon[0]=out->files[0]=0;
    ...
    line(text,"Icon",out->icon,sizeof(out->icon)); line(text,"Files",out->files,sizeof(out->files));
