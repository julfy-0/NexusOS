...
launch_nexus() {
    local display_backend="${NEXUS_QEMU_DISPLAY:-gtk,gl=off}"
    ...
    if [[ ! -s "$OUT_IMG" && -s "$OUT_ISO" ]]; then
        qemu-system-x86_64 -drive "if=pflash,format=raw,readonly=on,file=${ovmf_code}" -drive "if=pflash,format=raw,file=${ovmf_vars}" -cdrom "$OUT_ISO" -m "${MEM_MB}M" -device qemu-xhci,id=xhci -usb -device usb-tablet -cpu qemu64 -display "$display_backend"
    else
        ...
        qemu-system-x86_64 -drive "if=pflash,format=raw,readonly=on,file=${ovmf_code}" -drive "if=pflash,format=raw,file=${ovmf_vars}" -drive "format=raw,file=${OUT_IMG}" -m "${MEM_MB}M" -device qemu-xhci,id=xhci -usb -device usb-tablet "${accel[@]}" -display "$display_backend"
    fi
}
